...
export default function AdminDashboard() {
  ...
  // Filter business and supplier users from real-time location data
  const businessUsers: BusinessUser[] = userLocations.filter(loc => loc.userType === 'business').map(loc => ({
    id: loc.userId,
    name: loc.userName,
    businessType: loc.businessType || 'General Business',
    email: `contact@${loc.userName.toLowerCase().replace(/\s+/g, '')}.com`,
  ...
  const supplierUsers: SupplierUser[] = userLocations.filter(loc => loc.userType === 'supplier').map(loc => ({
    id: loc.userId,
    name: loc.userName,
    category: loc.category || 'General Supplier',
    email: `sales@${loc.userName.toLowerCase().replace(/\s+/g, '')}.com`,
  ...
  const centerPosition: [number, number] = [8.9475, 125.5406]; // Butuan City coordinates
  // Zone boundaries for Butuan City visualization
  const zones = [{
          ...
          <div className="p-4 lg:p-8 max-w-7xl mx-auto">
            ...
            {activeTab === 'users' && <div className="space-y-6">
              ...
              delay: 0.5
                  ...
                  {/* Map Container - Focused on Butuan City */}
