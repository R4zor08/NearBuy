...
// DEFAULT FALLBACK LOCATION: Tandag City coordinates (UPDATED TO PRECISE COORDINATES)
const TANDAG_CITY_CENTER: [number, number] = [9.0761231, 126.1967196];
const DEFAULT_ZOOM = 13; // City-level zoom for viewing major districts
...
// COMPREHENSIVE BUSINESS TYPES DATABASE
const BUSINESS_TYPES_DATABASE = [{
  name: 'Restaurant',
  icon: '🍽️',
  category: 'Food & Beverage',
  baseConfidence: [82, 95]
}, {
  name: 'Coffee Shop',
  icon: '☕',
  category: 'Food & Beverage',
  ...
  icon: '🍔',
  category: 'Food & Beverage',
  baseConfidence: [75, 88]
}, {
  name: 'Bakery',
  icon: '🥐',
  category: 'Food & Beverage',
  baseConfidence: [72, 86]
}, {
  ...
  icon: '🛒',
  category: 'Retail',
  baseConfidence: [80, 92]
}, {
  name: 'Pharmacy',
  icon: '💊',
  category: 'Healthcare',
  baseConfidence: [83, 94]
}, {
  name: 'Gym & Fitness',
  icon: '💪',
  ...
  baseConfidence: [76, 90]
}, {
  name: 'Tutoring Center',
  icon: '📚',
  category: 'Education & Care',
  baseConfidence: [73, 87]
}, {
  name: 'Veterinary Clinic',
  icon: '🐾',
  category: 'Pet Services',
  baseConfidence: [71, 84]
}, {
  name: 'Pet Store',
  icon: '🐕',
  category: 'Pet Services',
  baseConfidence: [68, 82]
}, {
  name: 'Laundromat',
  icon: '🧺',
  category: 'Services',
  baseConfidence: [77, 89]
}, {
  name: 'Car Wash',
  icon: '🚗',
  category: 'Automotive',
  baseConfidence: [69, 83]
}, {
  name: 'Auto Repair',
  icon: '🔧',
  category: 'Automotive',
  baseConfidence: [74, 87]
}, {
  name: 'Hardware Store',
  icon: '🔨',
  category: 'Retail',
  baseConfidence: [76, 88]
}, {
  name: 'Bookstore',
  icon: '📖',
  category: 'Retail',
  baseConfidence: [65, 79]
}, {
  name: 'Clothing Store',
  icon: '👕',
  category: 'Retail',
  baseConfidence: [70, 84]
}, {
  ...
  icon: '🖨️',
  category: 'Services',
  baseConfidence: [71, 85]
}, {
  name: 'Dental Clinic',
  icon: '🦷',
  category: 'Healthcare',
  baseConfidence: [75, 89]
}, {
  name: 'Medical Clinic',
  icon: '🏥',
  category: 'Healthcare',
...
// Generate random business recommendations for a location
const generateBusinessRecommendations = (locationId: string, count: number = 6) => {
  // Use location coordinates as seed for consistent randomization
  const seed = locationId.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
  // Shuffle business types based on seed
  const shuffled = [...BUSINESS_TYPES_DATABASE].sort((a, b) => {
    const hashA = (a.name.charCodeAt(0) + seed) % 100;
    const hashB = (b.name.charCodeAt(0) + seed) % 100;
    return hashB - hashA;
  });
  // Select top N businesses
  const selected = shuffled.slice(0, count);
  // Generate confidence scores within each business's range
  return selected.map((business, index) => {
    const [min, max] = business.baseConfidence;
...
// Generate detailed AI analysis for top recommendations
const generateAIAnalysis = (businessType: string, locationId: string) => {
  const seed = (businessType + locationId).split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
  // Generate realistic metrics based on seed
  const populationDensity = Math.floor(seed % 1500 + 1500); // 1500-3000 per km²
  const competitors = Math.floor(seed % 8 + 2); // 2-10 competitors
  const avgIncome = Math.floor(seed % 15000 + 25000); // ₱25k-40k
  const growthTrend = ['Growing', 'Stable', 'Emerging'][seed % 3];
  const trendDirection = growthTrend === 'Growing' ? 'up' : growthTrend === 'Stable' ? 'stable' : 'up';
  return {
    populationDensity: `${populationDensity.toLocaleString()} per km²`,
    populationLevel: populationDensity > 2200 ? 'High' : populationDensity > 1800 ? 'Medium' : 'Moderate',
    competitors: competitors,
    competitorLevel: competitors <= 3 ? 'Low' : competitors <= 6 ? 'Medium' : 'High',
    avgIncome: `₱${avgIncome.toLocaleString()}/month`,
    incomeLevel: avgIncome > 35000 ? 'Upper-Middle' : avgIncome > 28000 ? 'Middle' : 'Lower-Middle',
    growthTrend,
    trendDirection,
    marketGap: competitors <= 4 ? 'Significant opportunity' : competitors <= 7 ? 'Moderate opportunity' : 'Saturated market',
    reasoning: generateReasoning(businessType, competitors, populationDensity, avgIncome, growthTrend)
  };
};
// Generate AI reasoning text
const generateReasoning = (businessType: string, competitors: number, population: number, income: number, trend: string) => {
  const competitorText = competitors <= 3 ? `minimal competition (only ${competitors} similar businesses)` : competitors <= 6 ? `moderate competition (${competitors} similar businesses)` : `high competition (${competitors} similar businesses)`;
  const populationText = population > 2200 ? 'high population density' : population > 1800 ? 'moderate population density' : 'growing population density';
  const incomeText = income > 35000 ? 'strong purchasing power' : income > 28000 ? 'stable income levels' : 'developing income levels';
  return `${businessType} shows strong potential in this area due to ${populationText}, ${competitorText}, and ${incomeText}. Market analysis indicates ${trend.toLowerCase()} demand with favorable demographic trends.`;
};
export default function UnifiedMapView() {
  ...
  const [mapCenter, setMapCenter] = useState<[number, number]>(TANDAG_CITY_CENTER);
  const [baseLocation, setBaseLocation] = useState<{
    lat: number;
    lng: number;
  }>({
    lat: TANDAG_CITY_CENTER[0],
    lng: TANDAG_CITY_CENTER[1]
  });
  ...
  const [businessRecommendations, setBusinessRecommendations] = useState<any[]>([]);
  ...
  const [chatMessages, setChatMessages] = useState<{
    ...
    role: 'ai',
    content: "Hello! I'm SAFEBIZ.AI, your location insights assistant. How can I help you analyze this area?"
  }]);
  ...
  // Tandag City center coordinates (UPDATED TO PRECISE COORDINATES)
  const centerPosition: [number, number] = TANDAG_CITY_CENTER;
  // Zone boundaries for visualization - UPDATED TO REPRESENT REAL TANDAG CITY DISTRICTS
  const zones = [{
    // Downtown Commercial District (around Rizal Street, city center)
    bounds: [[9.073, 126.194], [9.079, 126.2]] as [[number, number], [number, number]],
    color: '#1E90FF',
    label: 'Downtown Commercial District',
    aiScore: 92,
    description: 'Main business hub around Rizal Street with shops, banks, and government offices'
  }, {
    // Northern Residential (Mabua, Awasian barangays)
    bounds: [[9.08, 126.192], [9.088, 126.2]] as [[number, number], [number, number]],
    color: '#36C275',
    label: 'Northern Residential Zone',
    aiScore: 0,
    description: 'Mabua and Awasian residential areas with growing communities'
  }, {
    // Eastern Areas (Bongtud, Pandanon)
    bounds: [[9.07, 126.2], [9.078, 126.208]] as [[number, number], [number, number]],
    color: '#F89C36',
    label: 'Eastern Mixed-Use Area',
    aiScore: 85,
    description: 'Bongtud and Pandanon areas with residential and commercial mix'
  }, {
    // Port/Industrial Zone (coastal area)
    bounds: [[9.068, 126.188], [9.074, 126.195]] as [[number, number], [number, number]],
    color: '#64748B',
    label: 'Port & Industrial Zone',
    aiScore: 78,
    description: 'Coastal port area with industrial and logistics facilities'
  }, {
    // Beach Recreation Areas (southern coastal)
    bounds: [[9.065, 126.192], [9.07, 126.198]] as [[number, number], [number, number]],
    color: '#A78BFA',
    label: 'Beach Recreation Area',
    aiScore: 88,
    description: 'Coastal recreation zone with beach resorts and tourism facilities'
  }];
    ...
    if (availableNames.length < count) {
      ...
      for (let i = 0; i < adjustedCount; i++) {
        ...
        newSuppliers.push({
          ...
          address: 'Tandag City Business District, Surigao del Sur',
  ...
  // MODIFIED: Initialize with Tandag City on mount (NO automatic GPS request)
  useEffect(() => {
    // Set Tandag City as default center
    const tandagCenter = TANDAG_CITY_CENTER;
    setMapCenter(tandagCenter);
    setBaseLocation({
      lat: tandagCenter[0],
      lng: tandagCenter[1]
    });
    // Generate initial suppliers for Tandag City
    const initialSuppliers = generateUniqueSuppliers(tandagCenter[0], tandagCenter[1], 5, 10);
  ...
  // MODIFIED: Track Location - Now opt-in only (no automatic GPS)
  const handleTrackLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(position => {
      ...
      // Update all location states
      setUserLocation(location);
      setMapCenter([location.lat, location.lng]);
      setBaseLocation(location);
      setIsTrackingLocation(true);
      setIsRequestingLocation(false);
      // Generate suppliers for user's location
      ...
      if (error.code === error.PERMISSION_DENIED) {
        toast.error('Location permission denied. Using Tandag City as default.', {
          duration: 5000
        });
      } else if (error.code === error.POSITION_UNAVAILABLE) {
        toast.error('Location information unavailable. Using Tandag City as default.');
      } else if (error.code === error.TIMEOUT) {
        toast.error('Location request timed out. Using Tandag City as default.');
      } else {
        toast.error('Unable to retrieve location. Using Tandag City as default.');
      }
    }, {
      enableHighAccuracy: true,
      timeout: 5000,
  ...
  // Generate business recommendations when location is selected
  useEffect(() => {
    if (selectedLocation) {
      const locationId = `${selectedLocation.lat}-${selectedLocation.lng}`;
      const recommendations = generateBusinessRecommendations(locationId, Math.floor(Math.random() * 4) + 5); // 5-8 recommendations
      setBusinessRecommendations(recommendations);
      // Generate detailed analysis for top 3
      const topThree = recommendations.slice(0, 3);
      const analysis = topThree.map(rec => ({
        businessType: rec.name,
        icon: rec.icon,
        confidence: rec.confidence,
        ...generateAIAnalysis(rec.name, locationId)
      }));
      setAiAnalysisData(analysis);
    }
  }, [selectedLocation]);
  // NEW: Chat bubble drag handlers
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    const maxX = window.innerWidth - (isChatOpen ? 384 : 64);
    const maxY = window.innerHeight - (isChatOpen ? 500 : 64);
    setChatPosition({
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      setChatMessages(prev => [...prev, {
        role: 'ai',
        content: 'I understand your question. Based on the current location analysis, I can provide insights about zone classifications, business opportunities, and market potential. What specific aspect would you like to explore?'
      }]);
    }, 1000);
    setChatInput('');
  };
  return <div className="space-y-6 pt-6 relative">
      ...
      y: 0
        ...
        <p className="text-m2m-text-secondary mt-2">
          ...
          placement. Default view: Tandag City.
        </p>
      </motion.div>
        ...
        <div className="lg:col-span-2">
          ...
          delay: 0.4
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="flex items-center space-x-3">
                  ...
                  {isLiveMode && isTrackingLocation && <motion.div animate={{
                  opacity: [1, 0.5, 1]
                }} transition={{
                  duration: 1.5,
                  repeat: Infinity
                ...
                <p className="text-xs text-m2m-text-secondary mt-1">
                  {isTrackingLocation ? 'Tracking your live location. Click anywhere to pin a location for insights.' : 'Default view: Tandag City (9.0761° N, 126.1967° E). Click "Track Location" to use your GPS or click anywhere to analyze a location.'}
                </p>
              </div>
              <div className="flex items-center space-x-2">
                ...
                scale: 0.95
              }} onClick={handleTrackLocation} disabled={isRequestingLocation} className={`px-4 py-2 rounded-lg transition-all flex items-center text-sm font-medium button-press ${isRequestingLocation ? 'bg-m2m-divider text-m2m-text-secondary cursor-not-allowed' : isTrackingLocation ? 'btn-nearbuy text-white shadow-nearbuy-md' : 'nearbuy-card hover:shadow-nearbuy-sm text-green-500'}`}>
            ...
            zIndex: 1
              ...
              {mapCenter && <MapContainer center={mapCenter} zoom={DEFAULT_ZOOM} scrollWheelZoom={true} style={{
              height: '100%',
              width: '100%'
            }} zoomControl={false} className="transition-all duration-500 ease-in-out" key={`map-${mapCenter[0]}-${mapCenter[1]}`}>
                  <SmoothZoom />
                  {/* Only enable location tracking when user explicitly enables it */}
                ...
                weight: 2
                  ...
                  {/* User Location Marker (Green GPS marker) - Only show when tracking */}
                  {userLocation && isTrackingLocation && <Marker position={[userLocation.lat, userLocation.lng]} icon={userLocationIcon}>
                      <Popup>
                        <div className="p-2">
                  ...
                  {selectedLocation && (!userLocation || !isTrackingLocation || selectedLocation.lat !== userLocation.lat || selectedLocation.lng !== userLocation.lng) && <Marker position={[selectedLocation.lat, selectedLocation.lng]} icon={selectedLocationIcon}>
                        <Popup>
                          <div className="p-2">
                            <h3 className="font-bold text-gray-900 mb-1">
                              Pinned Location
                  ...
                  {/* AI Recommended Markers - UPDATED TO TANDAG CITY LOCATIONS */}
                  {showAIRecommender && <>
                      <Marker position={[9.076, 126.197]} icon={aiRecommendedIcon}>
                        <Popup>
                          <div className="p-2">
                      ...
                      <Marker position={[9.074, 126.201]} icon={aiRecommendedIcon}>
                        <Popup>
                          <div className="p-2">
                            <div className="flex items-center space-x-2 mb-2">
                              <SparklesIcon className="w-5 h-5 text-blue-600" />
      ...
      {/* SAFEBIZ.AI Floating Chat Bubble */}
      <motion.div ref={chatBubbleRef} initial={{
      opacity: 0,
      scale: 0
    }} animate={{
      opacity: 1,
      ...
      type: 'spring',
      stiffness: 200
    }} className="fixed z-50 cursor-move" style={{
      bottom: chatPosition.y || 24,
      right: chatPosition.x || 24,
      left: chatPosition.x ? 'auto' : 'auto',
      top: chatPosition.y ? 'auto' : 'auto'
    }} onMouseDown={handleMouseDown}>
        <AnimatePresence mode="wait">
          {!isChatOpen ?
        // Collapsed bubble
        <motion.button key="bubble" initial={{
          scale: 0
        }} animate={{
          scale: 1
        }} exit={{
          ...
          scale: 0.9
        }} onClick={() => setIsChatOpen(true)} className={`w-16 h-16 rounded-full shadow-nearbuy-xl flex items-center justify-center relative overflow-hidden ${theme === 'dark' ? 'bg-gradient-to-br from-m2m-accent-blue to-m2m-accent-teal' : 'bg-gradient-to-br from-blue-500 to-teal-500'}`}>
              <motion.div animate={{
            rotate: [0, 10, -10, 0]
          }} transition={{
            duration: 2,
        ...
        <motion.div key="panel" initial={{
          ...
          type: 'spring',
          stiffness: 200
        }} className={`w-96 h-[500px] rounded-2xl shadow-2xl flex flex-col overflow-hidden ${theme === 'dark' ? 'bg-m2m-bg-card border border-m2m-divider' : 'bg-white border border-gray-200'}`} onClick={e => e.stopPropagation()}>
              {/* Header */}
              <div className={`p-4 flex items-center justify-between ${theme === 'dark' ? 'bg-gradient-to-r from-m2m-accent-blue to-m2m-accent-teal' : 'bg-gradient-to-r from-blue-500 to-teal-500'}`}>
                <div className="flex items-center space-x-3">
              ...
              scale: 0.9
            }} onClick={() => setIsChatOpen(false)} className="p-2 hover:bg-white/20 rounded-lg transition-colors">
                  <XIcon className="w-5 h-5 text-white" />
                </motion.button>
              </div>

              {/* Messages */}
              <div className={`flex-1 overflow-y-auto p-4 space-y-4 ${theme === 'dark' ? 'bg-m2m-bg-primary' : 'bg-gray-50'}`}>
                {chatMessages.map((message, index) => <motion.div key={index} initial={{
              opacity: 0,
              y: 10
            }} animate={{
              opacity: 1,
              ...
              delay: index * 0.1
                    ...
                    <div className={`max-w-[80%] rounded-2xl p-3 ${message.role === 'user' ? theme === 'dark' ? 'bg-gradient-to-r from-m2m-accent-blue to-m2m-accent-teal text-white' : 'bg-gradient-to-r from-blue-500 to-teal-500 text-white' : theme === 'dark' ? 'bg-m2m-bg-card border border-m2m-divider text-m2m-text-primary' : 'bg-white border border-gray-200 text-gray-900'}`}>
              ...
              {/* Input */}
              <div className={`p-4 border-t ${theme === 'dark' ? 'bg-m2m-bg-card border-m2m-divider' : 'bg-white border-gray-200'}`}>
                <div className="flex space-x-2">
                  <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSendMessage()} placeholder="Ask about this location..." className={`flex-1 px-4 py-2 rounded-lg focus:outline-none focus:ring-2 transition-all ${theme === 'dark' ? 'bg-m2m-bg-primary border border-m2m-divider text-m2m-text-primary placeholder-m2m-text-secondary focus:ring-m2m-accent-blue' : 'bg-gray-50 border border-gray-200 text-gray-900 placeholder-gray-400 focus:ring-blue-500'}`} />
                  <motion.button whileHover={{
                scale: 1.05
              }} whileTap={{
                scale: 0.95
              }} onClick={handleSendMessage} className={`p-2 rounded-lg transition-colors ${theme === 'dark' ? 'bg-gradient-to-r from-m2m-accent-blue to-m2m-accent-teal text-white' : 'bg-gradient-to-r from-blue-500 to-teal-500 text-white'}`}>
        ...
        {locationInsights && !isLoadingInsights && <>
          ...
          delay: 0.6
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Recommended Business Types - FIXED DIMENSIONS */}
                <div className="w-full h-[400px] flex flex-col bg-m2m-bg-card rounded-lg p-6 border border-m2m-divider">
                  <h3 className="text-lg font-semibold text-m2m-text-primary mb-4">
                    Recommended Business Types
                  </h3>
                  <div className="space-y-4 flex-1 overflow-y-auto">
                    {businessRecommendations.map((business, index) => <motion.div key={business.name} initial={{
                  opacity: 0,
                  x: -20
                }} animate={{
                  opacity: 1,
                  ...
                  delay: index * 0.1
                        ...
                        <div className="flex justify-between items-center mb-2">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-sm text-m2m-text-secondary font-medium">
                              {business.name}
                            </span>
                            <span className="text-xs px-2 py-0.5 bg-m2m-accent-blue/10 text-m2m-accent-blue rounded-full">
                              {business.category}
                            </span>
                          </div>
                          <motion.span initial={{
                      ...
                      opacity: 1
                    }} className={`text-sm font-bold ${business.confidence >= 85 ? 'text-m2m-success' : business.confidence >= 75 ? 'text-m2m-accent-blue' : 'text-m2m-accent-teal'}`}>
                            {business.confidence}%
                          </motion.span>
                        </div>
                      ...
                      ease: 'easeOut',
                      delay: index * 0.1
                    }} className={`h-full rounded-full ${business.confidence >= 85 ? 'bg-gradient-to-r from-m2m-success to-m2m-success/70' : business.confidence >= 75 ? 'bg-gradient-to-r from-m2m-accent-blue to-m2m-accent-blue/70' : 'bg-gradient-to-r from-m2m-accent-teal to-m2m-accent-teal/70'}`} />
                        </div>
                      </motion.div>)}
                  </div>
                </div>

                {/* Market Demand Levels - FIXED DIMENSIONS */}
                <div className="w-full h-[400px] flex flex-col bg-m2m-bg-card rounded-lg p-6 border border-m2m-divider">
                  <h3 className="text-lg font-semibold text-m2m-text-primary mb-4">
                ...
                {/* Growth Potential Metrics - FIXED DIMENSIONS */}
                <div className="w-full h-[400px] flex flex-col bg-m2m-bg-card rounded-lg p-6 border border-m2m-divider">
                  <h3 className="text-lg font-semibold text-m2m-text-primary mb-4">
                    Growth Potential
                  </h3>
            ...
            {/* NEW: AI Analysis Report Section */}
            {locationInsights && !isLoadingInsights && <motion.div initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          delay: 0.8
                ...
                <h2 className="text-2xl font-bold text-m2m-text-primary mb-6 flex items-center">
                  <BarChart3Icon className="w-6 h-6 mr-3 text-m2m-accent-teal" />
                  AI Analysis Report
                  <span className="ml-3 text-xs px-3 py-1 bg-m2m-accent-blue/20 text-m2m-accent-blue rounded-full font-semibold">
                    Top 3 Recommendations
                  </span>
                </h2>
                <p className="text-sm text-m2m-text-secondary mb-6">
                  Detailed evidence-based analysis for the highest-confidence
                  business opportunities at this location
                </p>

                <div className="space-y-6">
                  {aiAnalysisData.map((analysis, index) => <motion.div key={analysis.businessType} initial={{
              opacity: 0,
              y: 20
            }} animate={{
              opacity: 1,
              ...
              delay: index * 0.15
            }} className="bg-gradient-to-br from-m2m-accent-blue/5 to-m2m-accent-teal/5 rounded-xl p-6 border border-m2m-divider hover:border-m2m-accent-blue/50 transition-all">
                      {/* Header */}
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center space-x-3">
                          <div className="w-12 h-12 bg-gradient-to-br from-m2m-accent-blue to-m2m-accent-teal rounded-lg flex items-center justify-center text-2xl shadow-lg">
                            {analysis.icon}
                          </div>
                          <div>
                            <h3 className="text-xl font-bold text-m2m-text-primary flex items-center">
                              {analysis.businessType}
                              <span className="ml-2 text-xs px-2 py-1 bg-m2m-success/20 text-m2m-success rounded-full font-bold">
                                #{index + 1}
                              </span>
                            </h3>
                            <p className="text-sm text-m2m-text-secondary">
                        ...
                        <div className="flex items-center space-x-2">
                          {analysis.trendDirection === 'up' && <TrendingUpIcon className="w-5 h-5 text-m2m-success" />}
                          <span className={`text-xs font-bold px-3 py-1 rounded-full ${analysis.trendDirection === 'up' ? 'bg-m2m-success/20 text-m2m-success' : 'bg-m2m-chart-yellow/20 text-m2m-chart-yellow'}`}>
                  ...
                  scale: 1.05,
                  y: -2
                }} className="bg-m2m-bg-card rounded-lg p-4 border border-m2m-divider text-center">
                          <UsersIcon className="w-6 h-6 text-m2m-accent-blue mx-auto mb-2" />
                          <p className="text-xs text-m2m-text-secondary mb-1">
                            Population Density
                          </p>
                          <p className="text-sm font-bold text-m2m-text-primary">
                            {analysis.populationDensity}
                          </p>
                          <span className={`text-xs px-2 py-0.5 rounded-full mt-1 inline-block ${analysis.populationLevel === 'High' ? 'bg-m2m-success/20 text-m2m-success' : analysis.populationLevel === 'Medium' ? 'bg-m2m-accent-blue/20 text-m2m-accent-blue' : 'bg-m2m-chart-yellow/20 text-m2m-chart-yellow'}`}>
                            {analysis.populationLevel}
                          </span>
                        </motion.div>
                  ...
                  y: -2
                          ...
                          <TargetIcon className="w-6 h-6 text-m2m-accent-orange mx-auto mb-2" />
                          <p className="text-xs text-m2m-text-secondary mb-1">
                            Competitors Nearby
                          </p>
                          <p className="text-sm font-bold text-m2m-text-primary">
                            {analysis.competitors} businesses
                          </p>
                          <span className={`text-xs px-2 py-0.5 rounded-full mt-1 inline-block ${analysis.competitorLevel === 'Low' ? 'bg-m2m-success/20 text-m2m-success' : analysis.competitorLevel === 'Medium' ? 'bg-m2m-chart-yellow/20 text-m2m-chart-yellow' : 'bg-red-500/20 text-red-500'}`}>
                            {analysis.competitorLevel} Competition
                          </span>
                        </motion.div>
                  ...
                  y: -2
                          ...
                          <DollarSignIcon className="w-6 h-6 text-m2m-success mx-auto mb-2" />
                          <p className="text-xs text-m2m-text-secondary mb-1">
                            Avg Income
                          </p>
                          <p className="text-sm font-bold text-m2m-text-primary">
                          ...
                          <span className="text-xs px-2 py-0.5 bg-m2m-success/20 text-m2m-success rounded-full mt-1 inline-block">
                            {analysis.incomeLevel}
                          </span>
                        </motion.div>
                  ...
                  y: -2
                          ...
                          <SparklesIcon className="w-6 h-6 text-m2m-accent-teal mx-auto mb-2" />
                          <p className="text-xs text-m2m-text-secondary mb-1">
                            Market Gap
                          </p>
                          <p className="text-xs font-bold text-m2m-text-primary leading-tight">
                            {analysis.marketGap}
                          </p>
                          <CheckCircleIcon className="w-4 h-4 text-m2m-success mx-auto mt-1" />
                        </motion.div>
                      </div>

                      {/* AI Reasoning */}
                      <div className="bg-m2m-bg-card rounded-lg p-4 border border-m2m-accent-blue/30">
                        <div className="flex items-start space-x-3">
                          <div className="w-8 h-8 bg-gradient-to-br from-m2m-accent-blue to-m2m-accent-teal rounded-lg flex items-center justify-center flex-shrink-0">
                            <BotIcon className="w-5 h-5 text-white" />
                          </div>
                          <div>
                            <p className="text-xs font-bold text-m2m-accent-blue mb-1">
                              AI REASONING
                            </p>
                            <p className="text-sm text-m2m-text-secondary leading-relaxed">
                              {analysis.reasoning}
                            </p>
                          </div>
                        </div>
